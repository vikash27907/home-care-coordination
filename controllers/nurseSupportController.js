...
function createNurseSupportController() {
  ...
  createdCloudinaryAssetUrls.push(nextAadhaarBackUrl);
        ...
        console.log("SAVING PROFILE IMAGE:", nextProfileImageUrl);
...
END,
        ...
        try {
          await deleteNurseAssets(staleAssetUrls);
        } catch (cleanupError) {
          console.error("Unified nurse asset cleanup error:", cleanupError);
        }
  ...
  router.post("/nurse/profile/edit", requireRole("nurse"), requireApprovedNurse, async (req, res) => {
    ...
    try {
      if (files.profilePic && files.profilePic[0]) {
        const uploadedProfilePic = await uploadBufferToCloudinary(files.profilePic[0], "home-care/profile");
        setIfDefined("profile_image_url", uploadedProfilePic.secure_url);
      }
